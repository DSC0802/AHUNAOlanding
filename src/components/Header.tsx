  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        >
          AHUNAO
          <span className="text-orange-600 dark:text-orange-400">Caribe</span>
        </a>
        ...
        <nav className="hidden md:flex items-center space-x-6 lg:space-x-8">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-sm font-medium text-slate-700 hover:text-orange-600 dark:text-slate-200 dark:hover:text-orange-400 transition-colors"
            >
              {link.name}
            </a>
          ))}
